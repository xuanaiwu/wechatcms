var YDataGrid = function(config){
		...
		var Handler = {
			...
			edit:  function(callback){
				...
				if (Utils.checkSelectOne(record)){
					...
					dy.getById(Action.getId,data,function(result){
					...
					$.messager.confirm('Confirm','Are you sure you want to delete record?',function(r){  
					    if (r){
					    	...
					    	var  data = $("input[name='"+idKey+"']", Form.list ).fieldSerialize(); //序列化字段
					   		dy.deleteForm(Action.remove,data,function(result){
								Events.refresh();
								//回调函数
								if(jQuery.isFunction(callback)){
									callback(result);
			...
			save: function(callback){
				if(Form.edit.form('validate')){
					...
					dy.saveForm(Form.edit,function(data){
			...
			close : function (callback){
				$.messager.confirm('Confirm','你确定要关闭吗?',function(r){  
				    if (r){  
				     	Win.edit.dialog('close');
				     	//回调函数
						if(jQuery.isFunction(callback)){
		...
		var initTbar = function(){
			...
			//查询页面授权的btnType
			dy.ajaxJson(_url,data,function(data){
				if(data.success){
					if(data.allType){
